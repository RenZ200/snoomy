...
function showLogin(){if($('#edit-dialog').open)$('#edit-dialog').close();if(!$('#login-dialog').open)$('#login-dialog').showModal();}
...
async function refresh(){if(refreshing)return;refreshing=true;try{state=await api('/state');render();$('#sync').textContent='Tersimpan di server · sinkron tiap 10 detik';}catch(e){$('#sync').textContent='Belum tersambung · mencoba lagi';if(state.user)toast(e.message);}finally{refreshing=false;}}
